...
def featSelect(data, y):
    ...
    if len(miRNA_names_up)<2 or len(miRNA_names_down)<2:
        # print("no")
        return 0, 0, 0, 0, 0, [np.array([0,0,0,0]),0], 0, "There are no miRNAs with significant differences in expression"
    if len(miRNA_names_up)>20 or len(miRNA_names_down)>20:
        miRNA_names, miRNA_names_up, miRNA_names_down = RDA_new_6(data)
        if len(miRNA_names_up) < 2 or len(miRNA_names_down) < 2:
            miRNA_names, miRNA_names_up, miRNA_names_down = RDA_new_4(data)
